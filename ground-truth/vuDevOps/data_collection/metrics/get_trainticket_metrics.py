...
QUERIES = {
    ...
    "host_power": f"sum(rate(scaph_host_power_microwatts[{DURATION}])) by (instance) / 1000000",
}
...
def _exec_query(query, start_time, end_time, prom_url):
    ...
    for result in results:
        if all(
            k not in result["metric"].keys() for k in ["container", "name", "target", "container_label_com_docker_compose_service", "instance"]
        ):
            continue
        ...
        elif "instance" in result["metric"] and "host" in query:
            service_name = result["metric"]["instance"]
...
if __name__ == "__main__":
    ...
    print("GETTING TRAINTICKET METRICS")
