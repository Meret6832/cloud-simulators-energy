...
class RunnerConfig:
    ...
    def create_run_table_model(self) -> RunTableModel:
        ...
        factor2 = FactorModel("scenario", ['scenario_A'])
        factor3 = FactorModel("user_load", [100])
        repetitions = FactorModel("repetition_id", list(range(1, 2)))
        ...
        data_columns = [f"{service}_{metric}" for metric in metrics for service in services] + ["host_power"]
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        if system == 'sockshop':
            ...
            p.wait()
            p = subprocess.Popen('docker compose -f ../vuDevOps/microservices-demo/deploy/docker-compose/docker-compose.monitoring.yml up -d', shell=True)
            p.wait()
    ...
    def start_measurement(self, context: RunnerContext) -> None:
        ...
        print(f"start_measurement: {self.system} {scenario} {user_load} {repetition}")
    ...
    def run_cooldown(self, app_data, traffic_process):
        ...
        subprocess.run(["killall", ts_locust_script_process_name], check=False)
    ...
    def stop_run(self, context: RunnerContext) -> None:
        ...
        if self.system == 'sockshop':
            ...
            # # p.wait()
            p = subprocess.Popen('docker compose -f ../vuDevOps/microservices-demo/deploy/docker-compose/docker-compose.monitoring.yml stop', shell=True)
            p.wait()
        ...
        if self.system == 'trainticket':
            ...
            p.wait()
            p = subprocess.Popen('docker compose -f ../vuDevOps/microservices-demo/deploy/docker-compose/docker-compose.tsmonitoring.yml down', shell=True)
            p.wait()
    ...
    def populate_run_data(self, context: RunnerContext) -> Optional[Dict[str, Any]]:
        ...
        # host power
        # host_power_col = next((col for col in metrics_df.columns if "host_power" in col), None)
        # run_data["avg_host_power"] = round(metrics_df[host_power_col].mean(), 9)
