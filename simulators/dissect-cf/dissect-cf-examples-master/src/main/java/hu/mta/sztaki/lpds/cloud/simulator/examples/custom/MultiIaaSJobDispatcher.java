...
public class MultiIaaSJobDispatcher extends Timed {
	 ...
	 */
	public MultiIaaSJobDispatcher(GenericTraceProducer producer, List<IaaSService> target, Thread mainThread)
		...
		va = new VirtualAppliance("test", 0, 0, false, 100);
		...
		subscribe(minsubmittime - currentTime);
		if (verbosity) {
			new Thread() {
				...
				private void printStats() {
					printLog("subsrcibed=" + MultiIaaSJobDispatcher.this.isSubscribed() + " simTime="
				...
				public void run() {
                    ...
                    while (keepThread && !mainThread.isAlive()) {
						printStats();
	...
	public void tick(final long currTime) {
		...
		for (int i = minindex; i < jobs.length; i++) {
			...
			long submittime = toprocess.getSubmittimeSecs();
            // System.out.println("DEBUG checking job id=" + toprocess.getId());
			if (currTime == submittime) {
                // System.out.println("DEBUG trying to submit job id=" + toprocess.getId());
				...
				do {
					...
					if (requestedClouds <= target.size()) {
							...
							for (int j = 0; j < requestedClouds; j++) {
								...
								try {
									...
									for (int k = 0; k < currentRequestSize; k++) {
										VMKeeper newKeeper = new VMKeeper(currentTarget, vmsTemp[k], 3600);
                                    ...
                                    System.err.println("INCREASING IGNORECOUNTER");
									ignorecounter++;
								} catch (Exception e) {
						...
						if (servability) {
                            ...
                            System.err.println("not servable");
                            System.err.println("INCREASING IGNORECOUNTER");
							ignorecounter++;
						}
					} else {
