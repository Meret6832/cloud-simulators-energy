 ...
 */
public class TraceNetworkExperiment {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        try {
            ...
            hostList.add(new PowerNetworkHost(0,
            ...
            double schedulingInterval = 1; // The scheduling interval in seconds to update the processing of cloudlets
                                           // running in this VM.
            ...
            AppCloudlet app = new AppCloudlet(AppCloudlet.APP_Workflow, 0, experimentDuration*2, brokerId); // wrapper of cloudlets.
            ...
            PowerContainerNetworkDatacenter datacenter = createDatacenter("Datacenter", schedulingInterval, hostList, experimentName, vmSelectionPolicy, run, bw);

            // submit cloudlet list to the broker
            Log.println("Submitting Cloudletlist");

            // Load communication
            ArrayList<Double> comms = new ArrayList<>();
            String commTracePath = "../../traces/" + String.format("%s/%s/%d/%d/comms.data", system, scenario, users, repetitiion);
            BufferedReader commsInput = new BufferedReader(new FileReader(commTracePath));
            String commsLine;
            while ((commsLine=commsInput.readLine()) != null) {
                comms.add(Double.parseDouble(commsLine.trim()));
            }
            input.close();

            String commSizePath = "../../traces/" + String.format("%s/%s/%d/%d/comms_size.data", system, scenario, users, repetitiion);
            BufferedReader commsSizeInput = new BufferedReader(new FileReader(commTracePath));
            long commsSize = (long) Double.parseDouble(commsSizeInput.readLine().trim());

            // Communication
            UtilizationModel utilsSend = new UtilizationModelFull();
            UtilizationModel utilsRecv = new UtilizationModelFull();
            NetworkCloudlet clSend = new NetworkCloudlet(100, commsStep * (long) mips, 1, fileSize, outputSize, utilsSend, utilsSend, utilsSend);
            NetworkCloudlet clRecv = new NetworkCloudlet(101, commsStep * (long) mips, 1, fileSize, outputSize, utilsRecv, utilsRecv, utilsRecv);
            app.cList.add(clSend);
            cloudletList.add(clSend);
            app.cList.add(clRecv);
            cloudletList.add(clRecv);

            for (double c : comms) {
                for (int i=0; i<c; i++) {
                    // Sender sends request
                    clRecv.addRecvStage(clSend);
                    clSend.addSendStage(50, clRecv);

                    // Receiver sends response
                    clSend.addRecvStage(clRecv);
                    clRecv.addSendStage(commsSize, clSend);
                }
            }
    ...
    private static PowerContainerNetworkDatacenter createDatacenter(
            String name, double schedulingInterval, List<PowerNetworkHost> hostList, String experimentName, SelectionPolicy<GuestEntity> vmSelectionPolicy, int run, double bw) {
        ...
        try {
            ...
            String logAddress = String.format("outputs/trace-network/%d", run);
        ...
        CreateNetwork(datacenter, hostList.size(), bw);
    ...
    private static void CreateNetwork(PowerContainerNetworkDatacenter dc, double numHost, double b) {
        long bw = (long) b;
        Log.printlnConcat("bw ", bw);
        PowerSwitch ToRSwitch = new PowerSwitch("Edge0", numHost, PowerSwitch.SwitchLevel.EDGE_LEVEL, 0, (long) bw, (long) bw, dc);
