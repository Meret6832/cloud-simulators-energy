...
public interface HostEntity extends CoreAttributes {
    ...
    default boolean guestCreate(GuestEntity guest) {
        if (getStorage() < guest.getSize()) {
            Log.printlnConcat(CloudSim.clock(), "[GuestScheduler.guestCreate] Allocation of ", guest.getClassName(), " #",guest.getId(),
                    " failed by storage ", getStorage(), " < ", guest.getSize(), " ~", this.getClassName(), this.getId());
            return false;
        }
    ...
    default boolean isSuitableForGuest(GuestEntity guest) {
        // Log.printlnConcat("DEBUG (HostEntity) PEs ", getGuestScheduler().getPeCapacity() >= guest.getCurrentRequestedMaxMips());
        // Log.printlnConcat("DEBUG (HostEntity) mips ", getGuestScheduler().getAvailableMips() >= guest.getCurrentRequestedTotalMips(), " ", getGuestScheduler().getAvailableMips(), " >= ", guest.getCurrentRequestedTotalMips());
        // Log.printlnConcat("DEBUG (HostEntity) ram ", getGuestRamProvisioner().isSuitableForGuest(guest, guest.getCurrentRequestedRam()));
        // Log.printlnConcat("DEBUG  (HostEntity) bw ", getGuestBwProvisioner().isSuitableForGuest(guest, guest.getCurrentRequestedBw()));
