...
public class PowerDatacenter extends Datacenter {
	...
	public PowerDatacenter(
        ...
        // // Log.println("DEBUG (PowerDatacenter): creating PowerDatacenter");
	...
	protected void updateCloudletProcessing() {
		...
		if (currentTime > getLastProcessTime()) {
			...
			if (minTime != Double.MAX_VALUE) {
				CloudSim.cancelAll(getId(), new PredicateType(CloudActionTags.VM_DATACENTER_EVENT));
                // // Log.println("DEBUG (PowerDatacenter): sending");
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
            ...
            // // Log.println("DEBUG (PowerDatacenter)");

			Log.formatLine(
					"%.2f: [Host #%d] utilization is %f%%",
					currentTime,
					host.getId(),
