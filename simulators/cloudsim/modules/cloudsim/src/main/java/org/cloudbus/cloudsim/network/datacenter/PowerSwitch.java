...
import org.cloudbus.cloudsim.power.PowerNetworkHost;
  ...
  */
 public class PowerSwitch extends SimEntity {
      ...
      */
     public PowerContainerNetworkDatacenter dc;
     ...
     public PowerSwitch(String name, double numPort, SwitchLevel level, double switchingDelay, long downlinkBw, long uplinkBw, PowerContainerNetworkDatacenter dc) {
         super(name);
         this.level = level;
     ...
     protected void storePacketInHost(SimEvent ev) {
         ...
         PowerNetworkHost hs = hostList.get(npkt.receiverHostId);
     ...
     protected void processPacketUp(SimEvent ev) {
         ...
         if (level == SwitchLevel.EDGE_LEVEL) {
             int hostId = dc.VmtoHostlist.get(recvVMid);
             PowerNetworkHost hs = hostList.get(hostId);
             npkt.receiverHostId = hostId;
             ...
             PowerSwitch sw = uplinkSwitches.getFirst();
         ...
         // @TODO: confusing, this packet is going down, not up!!!
             ...
             int aggrSwitchId = -1;
             for (PowerSwitch sw : downlinkSwitches) {
                 for (PowerSwitch edge : sw.downlinkSwitches) {
     ...
     private void registerHost(SimEvent ev) {
         PowerNetworkHost hs = (PowerNetworkHost) ev.getData();
                 ...
                 for (NetworkPacket npkt : hspktlist) {
                    PowerNetworkHost hs = hostList.get(npkt.receiverHostId);
                     ...
                     if (hs == null) { // nested virtualization edge-case
                         for (PowerNetworkHost candidateHs: hostList.values()) {
